...
import { buildFindingQuizKey, isQuizEligibleFinding } from '../report-quiz';
...
const updateAnalysisReportSharingStatement = db.prepare(`
  ...
  WHERE id = ?
`);
const selectReportQuizSessionsByReportIdStatement = db.prepare(`
  SELECT *
  FROM report_quiz_sessions
  WHERE report_id = ?
  ORDER BY created_at DESC
`);
const selectReportQuizSessionByReportAndFindingStatement = db.prepare(`
  SELECT *
  FROM report_quiz_sessions
  WHERE report_id = ?
    AND finding_key = ?
`);
const selectReportQuizSessionByTokenStatement = db.prepare(`
  SELECT *
  FROM report_quiz_sessions
  WHERE session_token = ?
`);
const upsertReportQuizSessionStatement = db.prepare(`
  INSERT INTO report_quiz_sessions (
    report_id,
    user_id,
    finding_key,
    finding_title,
  ...
  ON CONFLICT(report_id, finding_key)
  DO UPDATE SET finding_title = excluded.finding_title,
...
`);
const markReportQuizSessionSolvedStatement = db.prepare(`
  UPDATE report_quiz_sessions
  SET status = 'solved',
      solved_at = ?,
      updated_at = ?
  WHERE session_token = ?
...
function safeParseJsonObject(value, fallback = null) {
  try {
...
export function formatAnalysisReport(row) {
  ...
  const rawFindings = safeParseJsonArray(row.findings_json);
  const quizSessions = listReportQuizSessionsByReportId(Number(row.id));
  const quizSessionByFindingKey = new Map(quizSessions.map((session) => [session.findingKey, session]));
  const findings = rawFindings.map((finding, index) => {
    const practiceKey = buildFindingQuizKey(finding, index);
    const session = quizSessionByFindingKey.get(practiceKey);
...
function formatReportQuizSession(row) {
  if (!row) {
    return null;
  }
  ...
  return {
    ...
    quiz: safeParseJsonObject(row.quiz_json, {}),
...
export function listReportQuizSessionsByReportId(reportId) {
  if (!Number.isInteger(Number(reportId))) {
    return [];
  }

  return selectReportQuizSessionsByReportIdStatement
    .all(Number(reportId))
    .map((row) => formatReportQuizSession(row))
    .filter(Boolean);
}

export function findReportQuizSessionByReportAndFinding(reportId, findingKey) {
  if (!Number.isInteger(Number(reportId))) {
    return null;
  }

  return formatReportQuizSession(
    selectReportQuizSessionByReportAndFindingStatement.get(Number(reportId), String(findingKey || '').trim())
  );
}

export function findReportQuizSessionByToken(sessionToken) {
  return formatReportQuizSession(selectReportQuizSessionByTokenStatement.get(String(sessionToken || '').trim()));
}

export function upsertReportQuizSession({
  reportId,
  userId,
  findingKey,
  findingTitle,
  ...
  upsertReportQuizSessionStatement.run(
    Number(reportId),
    Number(userId),
    String(findingKey || '').trim(),
    String(findingTitle || '').trim(),
    String(sessionToken || '').trim(),
    String(status || 'ready').trim(),
    JSON.stringify(quiz || {}),
    String(flagValue || '').trim(),
    generatedAt,
    solvedAt,
    timestamp,
    timestamp
  ...
  return findReportQuizSessionByReportAndFinding(reportId, findingKey);
}

export function markReportQuizSessionSolved(sessionToken) {
  const timestamp = now();
  markReportQuizSessionSolvedStatement.run(timestamp, timestamp, String(sessionToken || '').trim());
  return findReportQuizSessionByToken(sessionToken);
}
