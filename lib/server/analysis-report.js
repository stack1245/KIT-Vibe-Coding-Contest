...
function buildCodexFinalReportPrompt({
  ...
  return [
    ...
    'Report BOTH confirmed vulnerabilities AND suspected/potential vulnerabilities. Do not silently discard a finding just because evidence is partial.',
    'For suspected vulnerabilities, set severity to "medium" or "low" and clearly state in the detail field that this is a suspected issue requiring further verification.',
    'It is better to report 8 findings (some suspected) than to report only 2 confirmed findings. Coverage matters for an educational product.',
    'Survey the runtime broadly and let the actual entrypoints, trust boundaries, state transitions, and data flows determine which findings deserve attention.',
    ...
    'Do not skip low-severity issues such as verbose error messages, missing security headers, weak password policies, or insecure default configurations. Include them as low-severity findings.',
...
async function analyzeWithCodexExec({ acceptedFiles, contexts = [], onProgress, suspectedFindings = [] }) {
  ...
  const reconTimeoutMs = Math.max(8 * 60 * 1000, Math.floor(totalTimeoutMs * 0.38));
  const finalTimeoutMs = Math.max(14 * 60 * 1000, totalTimeoutMs - reconTimeoutMs);
...
function buildExpandedAbuse(rule, contexts) {
  ...
  return compactScenarioText(
    `1) 공격자가 ${sourceLabel}에 조작된 값을 넣는다. 2) 값이 ${validationText} 상태로 ${sinkLabel}까지 도달한다. 3) ${rule.detail}`,
