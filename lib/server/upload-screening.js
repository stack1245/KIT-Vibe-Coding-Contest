...
const APP_ROOT_DIR = /* turbopackIgnore: true */ process.cwd();

export const UPLOAD_ROOT_DIR = path.join(APP_ROOT_DIR, 'upload');
...
const NON_SERVICE_DATA_EXTENSIONS = new Set([
  '.adofai',
  '.osu',
  '.osz',
  '.qua',
...
const GAME_OR_LEVEL_DATA_PATTERNS = [
  /\bpathData\b/,
  /\bangleData\b/,
  /\btwirl\b/i,
  /\btrackColor\b/i,
  ...
  /\bbgImage\b/i,
  /\bartist\b/i,
  /\bsong(?:Filename|file|Name)?\b/i,
  /\bdifficulty\b/i,
  /\bpreview(?:Image|Icon|File)?\b/i,
  /\blevelDesc\b/i,
  /\blevelTags\b/i,
  /\bauthor\b/i,
  /\bchart\b/i,
  /\btiming(?:Points|Data)?\b/i,
  /\bbeat(?:map|s)?\b/i,
  /\blevel(?:Data|Path|Author|Desc|Tags)?\b/i,
  /\bbeatmap\b/i,
  /\brhythm\b/i,
];
const EXPLICIT_VULNERABILITY_PATTERNS = [
  { regex: /\bgets\s*\(/, signal: '명확한 버퍼 오버플로우 위험 함수' },
  { regex: /\bstrcpy\s*\(/, signal: '명확한 버퍼 오버플로우 위험 함수' },
  { regex: /\bsprintf\s*\(/, signal: '명확한 버퍼 오버플로우 위험 함수' },
  { regex: /\bscanf\s*\(\s*"(?:%s|%[^"]*s)/, signal: '길이 제한 없는 입력' },
  { regex: /\bfgets\s*\(\s*\(\s*char\s*\*\s*\)\s*\w+/, signal: '사용자 제어 주소 쓰기 가능성' },
  { regex: /\b(read|recv)\s*\(\s*\w+\s*,\s*\(\s*void\s*\*\s*\)\s*\w+/, signal: '사용자 제어 주소 쓰기 가능성' },
  { regex: /\bscanf\s*\(\s*"[^"]*(?:%lu|%lx|%lld|%p)[^"]*"\s*,\s*&\w+\s*\)/, signal: '주소 입력 primitive' },
  { regex: /\bprintf\s*\(\s*\w+\s*\)/, signal: '포맷 문자열 취약점 가능 구문' },
  { regex: /\b(system|popen|execve?)\s*\([^)]*(argv|input|buf|command|cmd|req|query|body)/i, signal: '명령 주입 가능 구문' },
  { regex: /\bSELECT\b[\s\S]{0,160}\+\s*\w+|\b(query|execute)\s*\([^)]*\+\s*\w+/i, signal: 'SQL Injection 가능 구문' },
  { regex: /\$where\b|\$regex\b|\$ne\b|\$gt\b|\$lt\b/i, signal: 'NoSQL Injection 가능 구문' },
  { regex: /\bstdin\b|\bstdout\b|\bstderr\b/i, signal: '표준 스트림 구조체 접근' },
];
const TEXT_PROGRAM_PROFILE_RULES = [
  {
    id: 'web-service',
    category: 'web-service',
    accepted: true,
    reason: '라우팅, 요청 처리, 인증, 세션, 데이터 저장 같은 웹 서비스 로직이 보여 분석 대상으로 허용했습니다.',
    patterns: [
      /\b(router\.(get|post|put|delete)|app\.(get|post|put|delete)|RequestMapping|RestController|fastapi|flask|express)\b/i,
      /\b(req|request|response|session|cookie|token|jwt|auth|login)\b/i,
      /\b(query|execute|SELECT|INSERT|UPDATE|DELETE|mongoose|sequelize|prisma)\b/i,
    ],
    minScore: 2,
  },
  {
    ...
    category: 'mobile-app',
    accepted: true,
    reason: '모바일 앱 컴포넌트, 권한, 저장소, 인텐트 같은 앱 로직이 보여 분석 대상으로 허용했습니다.',
    patterns: [
      /\b(AndroidManifest|Activity|Fragment|Intent|ContentProvider|Service)\b/i,
      /\b(UIKit|SwiftUI|UIViewController|AppDelegate|SceneDelegate)\b/i,
      /\b(sqlite|room|sharedpreferences|NSUserDefaults|Keychain)\b/i,
    ],
    minScore: 2,
  },
  {
    ...
    accepted: true,
    reason: '네이티브 프로그램이 입력, 상태, 메모리, 파일 또는 네트워크를 실제로 처리하는 로직이 보여 분석 대상으로 허용했습니다.',
    patterns: [
      /\b(int\s+main\s*\(|argc|argv|stdin|stdout|stderr)\b/i,
      /\b(malloc|calloc|realloc|free|memcpy|strcpy|fgets|read|recv|socket|open|fopen)\b/i,
      /\b(struct|typedef|menu|choice|create|delete|edit|update|show|list)\b/i,
    ],
    minScore: 2,
  },
  {
    id: 'parser-or-worker',
    category: 'parser-or-worker',
    accepted: true,
    reason: '파일 파싱, 직렬화, 변환, 압축 해제 같은 처리 로직이 보여 취약점 분석 가치가 있습니다.',
    patterns: [
      /\b(parse|parser|deserialize|serialize|decode|encode|tokenize|lexer|marshal|unmarshal)\b/i,
      /\b(json|xml|yaml|toml|protobuf|zip|tar|gzip|inflate|deflate)\b/i,
      /\b(load|save|import|export|extract|unpack|convert)\b/i,
    ],
    minScore: 2,
  },
  {
    id: 'challenge-bundle',
    category: 'challenge-bundle',
    accepted: true,
    reason: '취약점 실습 또는 CTF 성격의 코드/런타임 로직이 보여 분석 대상으로 허용했습니다.',
    patterns: [
      /\b(ctf|chall(?:enge)?|dreamhack|pwn|fsop|heap|uaf|double free|format string)\b/i,
      /\b(flag|win|shell|system|seccomp|alarm|ptrace)\b/i,
      /\b(run\.sh|dockerfile|libc|ld-linux|exploit|solve)\b/i,
    ],
    minScore: 2,
  },
];
...
function collectGameOrLevelSignals({ fileName, previewText }) {
  const extension = getFileExtension(fileName);
  const signals = [];

  if (NON_SERVICE_DATA_EXTENSIONS.has(extension)) {
    signals.push(`게임/레벨 데이터 확장자(${extension})`);
  }

  GAME_OR_LEVEL_DATA_PATTERNS.forEach((pattern) => {
...
function collectExplicitVulnerabilitySignals(text) {
  return Array.from(new Set(collectPatternSignals(text, EXPLICIT_VULNERABILITY_PATTERNS)));
}

function inferTextProgramProfile(text) {
  const scoredProfiles = TEXT_PROGRAM_PROFILE_RULES
    .map((rule) => {
...
function isLikelyTrivialTextProgram({ fileName, previewText, signals, hasLogic, explicitVulnerabilitySignals }) {
  if (explicitVulnerabilitySignals.length > 0 || signals.length > 0 || !hasLogic) {
    return false;
  }
  ...
  const codeLines = String(previewText || '')
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith('//') && !line.startsWith('#') && !line.startsWith('*'));
  const functionLikeMatches = String(previewText || '').match(/\b([A-Za-z_]\w*)\s*\(/g) || [];
  const uniqueFunctionLikeNames = new Set(
    functionLikeMatches
      .map((entry) => entry.replace(/\($/, '').trim())
      .filter(Boolean)
  );

  return codeLines.length <= 24 && uniqueFunctionLikeNames.size <= 5;
}
...
async function inspectBinaryPath({ displayName, tempPath }) {
  try {
    ...
    const explicitVulnerabilitySignals = collectExplicitVulnerabilitySignals(classificationText);
    ...
    if (!profileSupportsAcceptance && explicitVulnerabilitySignals.length === 0) {
      return {
        accepted: false,
    ...
    return {
      accepted,
      category: inferredProfile?.category || 'vulnerability-candidate-binary',
      reason: inferredProfile?.reason || '짧은 바이너리라도 명확한 취약점 primitive가 보여 취약점 분석 대상으로 허용했습니다.',
      source: 'rules',
      hardReject: false,
      confidence: 'high',
      previewText: evidenceText.slice(0, 12000),
...
async function inspectArchiveFile({ fileName, file }) {
  ...
  const isJarArchive = archiveExtension === '.jar';
  ...
  try {
    ...
    const gameOrLevelSignals = collectGameOrLevelSignals({ fileName, previewText });
    const archiveEvidenceText = `${previewText}\n${binaryPreviewText}\n${entryNames.join('\n')}`;
    const archiveProfile = inferTextProgramProfile(archiveEvidenceText);
    const archiveVulnerabilitySignals = collectExplicitVulnerabilitySignals(archiveEvidenceText);
    ...
    if (isJarArchive) {
      return {
        accepted: true,
        category: 'jar',
        reason: 'Java 아카이브(JAR)로 보이며 읽을 수 있어 분석 대상으로 유지했습니다.',
        source: 'rules',
        hardReject: false,
        confidence: 'high',
        previewText: [
        ...
        signals: [...signals, ...(archiveProfile ? [archiveProfile.id] : []), ...archiveVulnerabilitySignals],
      };
    }
    ...
    if (!hasLogic && (entrySummary.nonServiceDataFiles > 0 || gameOrLevelSignals.length >= 3)) {
      return {
        accepted: false,
        category: 'non-service-data-archive',
        reason: '압축 내부가 게임 레벨/맵 데이터 위주로 보여 실제 서비스 취약점 분석 대상에서 제외했습니다.',
        source: 'rules',
        hardReject: true,
        confidence: 'high',
        previewText,
        signals: [...signals, ...gameOrLevelSignals],
      };
    }
    ...
    const archiveAccepted = Boolean(archiveProfile?.accepted) || archiveVulnerabilitySignals.length > 0;

    return {
      accepted: archiveAccepted,
      category: archiveAccepted ? (archiveProfile?.category || 'analysis-archive-candidate') : 'uncertain-archive',
      reason: archiveAccepted
        ? (archiveProfile?.reason || '압축 파일 내부에서 실제 서비스 또는 취약점 분석 가치가 있는 프로그램 구성을 확인했습니다.')
        : '압축 파일 내부에 일부 파일은 있으나, 어떤 서비스나 분석 대상 프로그램인지 충분히 특정하지 못해 업로드하지 않았습니다.',
      source: 'rules',
      hardReject: false,
      confidence: archiveAccepted ? 'high' : 'high',
      previewText: [
        'ARCHIVE ENTRIES:',
        entryNames.slice(0, 200).join('\n'),
        '',
...
async function buildHeuristicScreeningResult({ fileName, previewBuffer, file }) {
  ...
  if (DOCUMENT_EXTENSIONS.has(extension)) {
    return {
      ...
      reason: '문서형 단일 파일은 분석 대상 로직으로 보기 어려워 업로드를 차단했습니다.',
      source: 'rules',
      hardReject: false,
      confidence: 'high',
    };
  ...
  if (ASSET_EXTENSIONS.has(extension)) {
    return {
      accepted: false,
      category: 'asset-only',
      reason: '이미지, 미디어, 폰트, 맵 자산 같은 단일 리소스 파일은 서비스 로직 분석 대상이 아니어서 업로드를 차단했습니다.',
      source: 'rules',
      hardReject: false,
      confidence: 'high',
    };
  ...
  const explicitVulnerabilitySignals = collectExplicitVulnerabilitySignals(previewText);
  const inferredTextProfile = inferTextProgramProfile(previewText);
  const presentationOnly = isLikelyPresentationOnly({ fileName, previewText });
  const gameOrLevelSignals = collectGameOrLevelSignals({ fileName, previewText });
  const trivialTextProgram = isLikelyTrivialTextProgram({
    fileName,
    previewText,
    signals,
    hasLogic,
  ...
  const accepted = Boolean(inferredTextProfile?.accepted) || explicitVulnerabilitySignals.length > 0;

  if (gameOrLevelSignals.length >= 3 && !signals.length && !hasLogic) {
    return {
      accepted: false,
      category: 'non-service-data',
      reason: '서비스 코드가 아니라 게임/레벨 데이터로 판단되어 업로드를 차단했습니다.',
      source: 'rules',
      hardReject: true,
      confidence: 'high',
      previewText,
      signals: gameOrLevelSignals,
    };
  }
  ...
  if (trivialTextProgram) {
    return {
      accepted: false,
      category: 'trivial-text-program',
      reason: '함수 몇 개만 있는 최소 예제로 보이며, 서비스 로직이나 명확한 취약점 근거가 부족해 업로드를 차단했습니다.',
      source: 'rules',
      hardReject: false,
      confidence: 'high',
      signals,
  ...
  return {
    accepted,
    category: accepted ? (inferredTextProfile?.category || 'application-source') : 'not-an-app',
    reason: accepted
      ? (inferredTextProfile?.reason || '어떤 서비스나 분석 대상 프로그램인지 대강 특정할 수 있어 업로드를 허용했습니다.')
      : '이 파일이 실제로 어떤 서비스나 분석 대상 프로그램인지 충분히 특정하지 못해 업로드하지 않았습니다.',
    source: 'rules',
    hardReject: false,
    confidence: 'high',
    signals: [...signals, ...(inferredTextProfile ? [inferredTextProfile.id] : []), ...explicitVulnerabilitySignals],
    previewText,
  };
}
...
async function screenWithCodexExec({ fileName, contentType, previewText, heuristic }) {
  ...
  const prompt = [
    ...
    '',
    'Your task is to filter out only clearly meaningless or low-value uploads. Be permissive. If a file could plausibly be worth security review, accept it.',
    '',
    'Core principle:',
    '- Reject only files that are clearly useless for vulnerability analysis.',
    '- If a file plausibly contains real logic, executable behavior, parsing behavior, service behavior, app behavior, or externally influenced processing, accept it.',
    '- If uncertain, accept.',
    '- Respond with JSON only.',
    '',
    'Special rule for JAR files:',
    '- Always accept JAR files for analysis.',
    '- Do not reject a file just because it is a .jar.',
    '- Treat every JAR as potentially meaningful unless it is explicitly and clearly invalid or unreadable.',
    '- Main-Class, package names, framework traces, partial class lists, or incomplete structure are enough to keep it.',
    '- Do not require proof of runtime behavior, network exposure, or a real vulnerability for JAR acceptance.',
    '- For JAR files, default to accepted=true.',
    '',
    'Accept when the artifact plausibly appears to be:',
    '- a binary or executable with non-trivial logic',
    '- a web/backend/API component',
    '- a desktop or mobile app',
    '- a network service or protocol handler',
    '- a parser, extractor, importer, converter, or decoder',
    '- a CLI tool that processes files, arguments, or external input',
    '- a library/component that handles untrusted input',
    '- a plugin/module/agent/worker with meaningful logic',
    '- an archive/package/app bundle that likely contains a real application',
    '- any JAR file',
    '',
    'Reject only when the artifact is clearly one of these:',
    '- image',
    '- font',
    '- stylesheet',
    '- static_asset',
    ...
    '- sample_output',
    '- trivial_demo',
    '- resource_bundle_only',
    '- dependency_manifest_only',
    '',
    'Binary guidance:',
    '- Do not reject a binary only because evidence is incomplete.',
    '- Do not require proof of a vulnerability.',
    '- Do not require proof of network exposure.',
    '- Generic imports alone are weak evidence, but if the file plausibly looks like a real executable program, accept it.',
    '- Reject only if it clearly looks trivial, inert, or non-program-like.',
    '',
    'How to set suspicious:',
    '- suspicious=true only when there are concrete signs of risky functionality worth prioritizing later',
    '- suspicious=false for ordinary accepted files with no strong risk signal yet',
    '- if accepted=false, suspicious must be false',
    '',
    'Reason requirements:',
    'Keep the reason short and specific.',
    'State:',
    '1. what the artifact most likely is',
    '2. why it was accepted or rejected',
    '',
    'Output:',
    'Respond with JSON only.',
  ...
  try {
    ...
    return {
      ...
      reason,
      source: 'codex',
      suspicious: parsed.accepted ? Boolean(parsed.suspicious) : false,
      hardReject: false,
      confidence: 'low',
    };
  } catch {
...
export async function saveUploadedFile({ userId, file, originalName }) {
  ...
  const relativePath = normalizeStoredUploadRelativePath(path.relative(UPLOAD_ROOT_DIR, absolutePath));
