...
export async function sendVerificationEmail(email, code) {
  return sendCodeEmail({
    ...
    title: 'Phase Vuln Coach 이메일 인증',
    description: '아래 인증 코드를 3분 이내에 입력해주세요.',
  });
}
...
export async function sendPasswordResetEmail(email, code) {
  return sendCodeEmail({
    email,
    code,
    subject: '[Phase Vuln Coach] 비밀번호 재설정 코드',
    title: 'Phase Vuln Coach 비밀번호 재설정',
    description: '아래 인증 코드를 입력한 뒤 새 비밀번호를 설정해주세요.',
  });
}

async function sendCodeEmail({ email, code, subject, title, description }) {
  ...
  await transporter.sendMail({
    ...
    subject,
    text: `${title}: ${code}\n${description}`,
    html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#111"><h2>${title}</h2><p>${description}</p><p style="font-size:28px;font-weight:700;letter-spacing:0.18em">${code}</p></div>`,
  });
