...
function buildCodexFinalReportPrompt({
  ...
  return [
    ...
    'The title must be service-type-centric, not vulnerability-name-centric. Use concrete service categories such as "모바일 쇼핑앱 취약점 리포트", "영상 변환 웹 서비스 취약점 리포트", or "코드 업로드와 GitHub 저장소 취약점 분석 웹 서비스 취약점 리포트".',
    'applicationReport is the content for a section titled "서비스 구조 요약".',
    'In applicationReport, the first sentence must explain the service in plain language from a normal user perspective. After that, continue with developer-facing technical explanation about endpoints, handlers, functions, and processing flow.',
    'Do not talk about analysis steps in applicationReport. Describe the service first, then its technical structure.',
    'For each finding, explanation is the content for "취약점 설명", detail is the content for "취약점 원인 분석", location/codeLocation is used for "파일 경로 및 핵심 코드", remediation is the content for "취약점 해결 방안", and poc is the content for "패치 코드 예시".',
    'Do not include sections such as "원인", "공격 시나리오", "등급 근거", or "익스 영향".',
    'The explanation field must be long, educational, and lecture-like, focusing on the vulnerability concept itself.',
    'The detail field must trace the real code flow and explain exactly where, how, and why the vulnerability exists in the current program.',
    'The remediation field must explain practical fixes in detail, including why the fix works.',
    ...
    'The poc field must contain a patch example section with both "현재 코드:" and "패치 예시 코드:". Show the current vulnerable code first, then the improved patched example.',
    ...
    JSON.stringify({
      ...
      constraints: [
        'title must end with "취약점 리포트"',
        'title must describe the service category, not the vulnerability name',
        'applicationReport must begin with a user-facing service explanation and then continue with a developer-facing technical explanation',
        ...
        'findings must include BOTH confirmed and suspected vulnerabilities. Aim for 3-5 findings if the codebase has any complexity.',
        'for suspected findings, detail must clearly state "의심 단계" or "추가 검증 필요" so the reader understands the confidence level',
        'explanation must be a detailed educational description of the vulnerability itself',
        'detail must explain the current code-flow root cause in detail',
        'remediation must be practical, detailed, and educational',
        'poc must contain both "현재 코드:" and "패치 예시 코드:"',
      ],
      uploadedFiles: manifest,
    }, null, 2),
  ].join('\n\n');
...
function buildCodexFinalRescuePrompt({ manifest, reconnaissance }) {
  return [
    ...
    'The title must be service-type-centric and end with "취약점 리포트".',
    'applicationReport is the content for a section titled "서비스 구조 요약". Start with a user-facing service summary and then continue with concise developer-facing technical explanation.',
    'For each finding, explanation is "취약점 설명", detail is "취약점 원인 분석", location/codeLocation is "파일 경로 및 핵심 코드", remediation is "취약점 해결 방안", and poc is "패치 코드 예시".',
    'Do not include sections such as "원인", "공격 시나리오", "등급 근거", or "익스 영향".',
    'The poc field must contain both "현재 코드:" and "패치 예시 코드:".',
    ...
    JSON.stringify({
      ...
      constraints: [
        ...
        'title must end with "취약점 리포트"',
        'applicationReport must begin with a user-facing service summary and then continue with a developer-facing technical explanation',
        'poc must contain both "현재 코드:" and "패치 예시 코드:"',
      ],
      uploadedFiles: manifest,
    }, null, 2),
