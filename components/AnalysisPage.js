...
import { deleteAnalysisReportAction, toggleAnalysisReportShareAction } from '../app/analysis/actions';
...
export default function AnalysisPage({ initialReports = [], preferences = null, github = null }) {
  ...
  async function handleDeleteReport() {
    ...
    try {
      await deleteAnalysisReportAction(reportToDelete.id);
  ...
  async function handleCopyShare(report) {
    try {
      ...
      if (!report.shareEnabled) {
        ...
        const payload = await toggleAnalysisReportShareAction(report.id, true, window.location.origin);
                      ...
                      return (
                        <div key={report.id} className={styles.resultItem}>
                          ...
                          <div className={styles.resultBottom}>
                            ...
                            <div className={styles.miniChip}>탐지 {report.findingsCount}개</div>
          ...
          <div className={styles.reportModal} role="dialog" aria-modal="true">
            ...
            <section className={styles.reportSection}>
              ...
              <p className={styles.reportParagraph}>
                {activeReport.resultMode === 'vulnerability'
                  ? `취약점이 ${activeReport.findingsCount}개 발견되었습니다.`
                  : '확정 취약점 대신 보안적으로 보완할 지점을 추천합니다.'}
              </p>
